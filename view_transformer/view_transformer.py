...
class ViewTransformer:
    def __init__(self):
        ...
        self.pixel_vertices = self.pixel_vertices.astype(dtype=np.float32)
        self.target_vertices = self.target_vertices.astype(dtype=np.float32)
        ...
        for object_s,object_tracks in tracks.items():
            for frame_no,track in enumerate(object_tracks):
                for track_id,track_info in track.items():
                    ...
                    tracks[object_s][frame_no][track_id]['position_transformed'] = transformed_position
